...
class ForexEnv7(gym.Env):
    ...
    def __init__(self, **kwargs):
        ...
        self.observation_space = spaces.Box(low=float(-1.0), high=float(1.0),
                                            shape=(self.obs_ticks, 1, self.num_features), dtype=np.float32)
        self.order_time = 0
    ...
    def step(self, action):
        ...
        if (self.episode_over == False):
            ...
            if self.profit_pips <= (-1 * self.sl):
                ...
                # print transaction: Num,DateTime,Type,Size,Price,SL,TP,Profit,Balance
                if self.debug == 1:
                    print(self.tick_count, ',stop_loss, pips:', self.profit_pips, ' profit:', self.real_profit, ',b:',
                          self.balance)
            ...
            if self.profit_pips >= self.tp:
                ...
                # print transaction: Num,DateTime,Type,Size,Price,SL,TP,Profit,Balance
                if self.debug == 1:
                    print(self.tick_count, ',take_profit, pips:', self.profit_pips, ' profit:', self.real_profit, ',b:',
                          self.balance)
            ...
            if (self.order_status == 0) and action[3] > 0:
                ...
                if self.debug == 1:
                    print(self.tick_count, ',buy, o', self.open_price, ',v', self.order_volume, ' tp:', self.tp, ' sl:',
                          self.sl, ' b:', self.balance)
            ...
            if (self.order_status == 0) and action[3] < 0:
                ...
                # print transaction: Num,DateTime,Type,Size,Price,SL,TP,Profit,Balance
                if self.debug == 1:
                    print(self.tick_count, ',sell, o', self.open_price, ',v', self.order_volume, ' tp:', self.tp,
                          ' sl:', self.sl, ' b:', self.balance)
            ...
            if ((self.tick_count - self.order_time) > self.min_order_time):
                ...
                if (self.order_status == -1) and action[3] > 0:
                    ...
                    # print transaction: Num,DateTime,Type,Size,Price,SL,TP,Profit,Balance
                    if self.debug == 1:
                        print(self.tick_count, ',close_sell, pips:', self.profit_pips, ' profit:', self.real_profit,
                              ',b:', self.balance)
                ...
                if (self.order_status == -1) and action[3] == 0:
                    print(self.tick_count, ',o_sell, pips:', self.profit_pips, ' profit:', self.real_profit, ',b:',
                          self.balance)
                # print("action=", action)
                ...
                if (self.order_status == 1) and action[3] < 0:
                    ...
                    # print transaction: Num,DateTime,Type,Size,Price,SL,TP,Profit,Balance
                    if self.debug == 1:
                        print(self.tick_count, ',close_buy, pips:', self.profit_pips, ' profit:', self.real_profit,
                              ',b:', self.balance)
                    # Set closing cause 0 = normal close
                    self.ant_c_c = self.c_c
                    self.c_c = 0
                    ...
                    self.num_closes += 1
                if (self.order_status == 1) and action[3] == 0:
                    print(self.tick_count, ',o_buy, pips:', self.profit_pips, ' profit:', self.real_profit, ',b:',
                          self.balance)
        ...
        if self.reward_function == 0:
            ...
            if (self.num_closes < self.min_orders) and reward <= 0:
                reward = reward - (
                            (self.initial_capital / (10 * self.num_ticks)) * (1 - (self.num_closes / self.min_orders)))
        ...
        info = {"balance": self.balance, "tick_count": self.tick_count, "order_status": self.order_status,
                "num_closes": self.num_closes, "equity": self.equity}
